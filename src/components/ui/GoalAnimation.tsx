...
const GoalAnimation: React.FC<GoalAnimationProps> = ({ 
  ...
  if (type === 'GAME_OVER') {
    ...
    return (
      <motion.div 
        ...
        className="absolute inset-0 z-50 bg-black/95 backdrop-blur-md flex flex-col items-center justify-center"
      >
        <motion.h1 
  ...
  return (
    <motion.div 
        ...
        className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-sm p-8 bg-black/95 z-50 rounded-3xl flex flex-col items-center justify-center border-4 ${borderColor} shadow-2xl`}
    >
      <motion.h1 
      ...
      {/* Affichage du score pendant l'animation du but */}
